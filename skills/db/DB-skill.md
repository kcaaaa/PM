...
# 数据库设计技能 · Base 约束下的数据领域能力

本技能提供全面的数据库设计指导，涵盖从需求分析到物理设计的完整流程，帮助构建高效、可靠、可扩展的数据库系统。

> **定位**：本 Skill 是数据库领域专家，服务于 `Base` 对象包链路，不是独立数据库实施流程。
>
> **核心原则**：数据库设计、Schema 调整、SQL 模板、迁移脚本与性能优化建议，必须服从 `RQ / PD / TD / FT / IMP / CR / TEST` 的追溯关系。

---

## Base 前置判断

| 场景 | 必要 Base 关联 | 是否可直接输出落地 SQL / 迁移脚本 |
|---|---|---|
| 数据库概念、范式、索引、SQL 写法咨询 | 可不绑定 | 可给示例，不得当作项目可执行脚本 |
| 新业务数据建模、表结构设计 | `RQ-*`、`PD-*`、`TD-*` | 否，应先写入技术方案 |
| 功能级字段、索引、约束设计 | `RQ-*`、`FT-*`、`TD-*` | 否，应形成方案和验收标准 |
| 直接创建/修改表、数据迁移、生产 SQL | `RQ-*`、`FT-*`、`TD-*`、`IMP-*` | 仅在 `IMP-*` 已确认且用户明确要求执行/产出时允许 |
| 性能优化涉及结构变更 | `TD-*`、`IMP-*`，必要时 `CR-*` | 仅允许输出经影响分析后的增量方案 |
| 数据变更影响产品逻辑或用户体验 | `CR-*` | 否，先进入变更影响分析 |
| 数据验证、回归测试、压测 | `TEST-*` | 可输出测试 SQL / 验证清单 |

**硬规则**：无 `TD-*` 或 `IMP-*` 时，不得把示例 SQL 包装成项目可执行方案；涉及数据结构、数据迁移、生产性能优化的内容必须先落入 Base 对象包。

> **使用约束**：本文件仅负责说明“什么时候该读哪些子文件”，不承载各场景的完整方法论；具体规范应按需跳转至子文件。
...
| 输出内容 | 写入位置 |
|---|---|
| 数据需求、实体识别、数据边界 | `Base/01-需求管理/RQ-*/需求说明.md` 或 `Base/03-产品设计/PD-*/数据对象.md` |
| 数据库选型、ER 模型、表结构方案 | `Base/04-方案设计/TD-*/数据库设计.md` |
| 功能级数据规则、字段约束、验收口径 | `Base/05-功能管理/FT-*/验收标准.md` |
| 建表脚本、迁移脚本、回滚方案、执行顺序 | `Base/06-实现管理/IMP-*/数据库实现.md` |
| 数据验证、性能验证、回归测试 | `Base/09-测试验收/TEST-*/测试用例.md` |
| Schema 变更、数据口径变化、兼容性影响 | `Base/08-变更管理/CR-*/影响分析.md` |

---

## 按需跳读索引（优先使用，避免全文加载）

> 本文件内容较长（~460 行）。**若用户诉求明确，优先按下表直接跳到对应小节或子文件**，不要把整篇全读进上下文。

| 用户诉求 | 跳到 |
|---|---|
| 什么是主键/外键/范式/索引？（基础概念） | 本文 §核心概念（约 10-80 行） |
| 如何从需求走到建表？（方法论） | 本文 §设计方法论（约 85-170 行） |
| 建表规范 / 命名 / 审计字段 | 本文 §最佳实践 · 表设计规范 + 专用子文件 `DB-skills/naming-conventions.md` |
...
**完整流程场景**（新建数据库、业务建模）才需要从 §核心概念 读到 §数据库类型选择。
