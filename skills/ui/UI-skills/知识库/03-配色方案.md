# 知识库 · 03 配色方案（96 套产品级配色）

> **来源**：由 `colors.csv` 完整转译。
> **作用**：根据产品类型直接查一组**可投产的 5 色方案**（Primary / Secondary / CTA / Background / Text），附 Border 建议色与使用 Notes。
> **搜索字段**：`Product Type / Notes`

---

## 使用提示

1. 先从《01-产品-风格映射》拿到 `UI_Category`；
2. 在下表找**同名产品**（若无，用相邻类目回退：如 `Analytics Dashboard` 无精确匹配则参考 `SaaS Dashboard`）；
3. 把 5 个 hex 映射到项目主题变量（CSS `:root` / Tailwind `theme.extend.colors`）。
4. Border 列的 hex 是浅色模式下卡片 / 输入框的推荐边框色。
...
| 查询类目 | 无精确匹配时回退到 |
|---|---|
| SaaS Dashboard | `#1` SaaS (General) 或 `#8` Analytics Dashboard |
| B2B SaaS Enterprise | `#1` SaaS (General) 或 `#6` B2B Service |
| Healthcare（非 App） | `#61` Medical Clinic 或 `#9` Healthcare App |
| Wellness/Mental Health（同义） | `#24` Mental Health App |
| Luxury（通用） | `#35` Luxury/Premium Brand 或 `#4` E-commerce Luxury |
| Startup Landing | `#5` Service Landing Page |
| Fintech (Banking) | `#44` Banking/Traditional Finance |
| Gaming（AAA） | `#13` Gaming |
| Travel | `#39` Travel/Tourism Agency |
| 任意未列出 | 最后兜底 `#1` SaaS (General) |

---

## Part C · 深色模式 / 浅色模式识别

**已内置深色背景的方案**（Background 偏深）：
- `#7` Financial Dashboard（`#020617`）
- `#13` Gaming（`#0F0F23`）
- `#15` Fintech/Crypto（`#0F172A`）
- `#20` NFT/Web3（`#0F0F23`）
- `#26` Smart Home/IoT（`#0F172A`）
- `#29` Podcast（`#0F0F23`）
- `#37` Fitness/Gym（`#1F2937`）
- `#47` Music Streaming（`#0F0F23`）
- `#48` Video Streaming/OTT（`#000000`）
- `#55` Photography Studio（`#000000`）
- `#83` Theater/Cinema（`#0F0F23`）
- `#85` Coding Bootcamp（`#020617`）
- `#86` Cybersecurity（`#000000`）
- `#87` Developer Tool/IDE（`#0F172A`）
- `#89` Space Tech/Aerospace（`#0B0B10`）
- `#91` Quantum Computing（`#050510`）
- `#93` Autonomous Systems（`#0D1117`）

> 其余方案默认以**浅色**为主背景；Text 列是浅色模式下的文字色。
> 若项目需要双主题切换，可把 Primary / CTA / Secondary 保留，Background / Text 对调（例如 `#1` SaaS 双主题时 Dark 模式 bg 用 `#020617`，text 用 `#F8FAFC`）。

---

## Part D · CSS 变量生成片段

以 `#1 SaaS (General)` 为例，可直接粘进项目：
...
1. **文字对比度 ≥ 4.5:1（AA）**；医疗 / 政府 / 老年护理场景 ≥ 7:1（AAA）。
2. **CTA 颜色必须是可访问对比色**——Primary 作为背景时，CTA 的前景文字必须过对比度。
3. **Dark 背景方案下禁用纯白文字**——用 `#F8FAFC` / `#E0E0E0` 减眼部压力。
4. **Gold 类强调色（`#CA8A04` / `#FFD700` / `#D4AF37`）** 仅用于奢华 / 金融 / 信任场景，切勿滥用。
5. **红色**（`#DC2626` / `#E11D48` / `#FF3333`）严禁用于主色——除非是本身就要表达"紧急 / 警报"的领域（Cybersecurity / News Breaking / Fire Emergency）。
6. **AI 紫粉渐变**（`#6366F1 → #EC4899`）不可用于以下反模式类目（详见《01-产品-风格映射》Part B.3）：政府、医疗、银行、保险、老年护理、药店、医疗诊所、法律、咨询、建筑、农业、物流、B2B 服务、B2B SaaS Enterprise。
